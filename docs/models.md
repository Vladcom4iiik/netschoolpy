...
## MailEntry (Краткая запись о письме)

Одна строка из списка писем (возвращается из `mail_list()`).

- `id` (int): ID письма.
- `subject` (str): Тема.
- `author` (str): Автор (ФИО + организация).
- `sent` (datetime.datetime): Дата отправки.
- `to_names` (str | None): Кому адресовано (заполняется для папки «Отправленные»).

## MailPage (Страница списка писем)

Результат `mail_list()` — страница с записями и информацией о пагинации.

- `entries` (List[MailEntry]): Список писем на текущей странице.
- `page` (int): Номер текущей страницы.
- `total_items` (int): Общее количество писем в папке.
