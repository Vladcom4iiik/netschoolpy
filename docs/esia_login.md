...
async def main():
    ...
    async with NetSchool(url) as ns:
        # Вход через ESIA
        await ns.login_via_gosuslugi('ваш_телефон_или_email', 'пароль_от_госуслуг')
...
!!! warning "Важно"
    ...
    Если у вас включена **двухфакторная аутентификация (2FA)** на Госуслугах, библиотека поддерживает SMS, TOTP (приложение-аутентификатор) и push (Госключ). Код будет запрошен через `input()`.
